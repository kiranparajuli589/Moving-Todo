...
module.exports = {
  ...
  commands: {
    ...
    todoWithSubjectAndContentShouldBeVisible(subject, content) {
      return this.api.element(
        "xpath",
        util.format(
          this.elements.todoBoxTitleContent.selector,
          subject,
          content
        ),
        (result) => {
          if (result.value.message === undefined) {
            ...
            throw new Error(
              "Could not find the todo with provided information"
            );
          }
        }
      );
    ...
    todoWithEnteredSubjectAlreadyCreated(subject) {
      return this.api.element(
        "xpath",
        util.format(this.elements.todoBoxTitle.selector, subject),
        (result) => {
          if (result.value.message === undefined) {
    ...
    todoWithDesiredSubjectsAreListedInAutocompleteMenu(callback) {
      return this.waitForElementVisible("@searchSuggestions").getText(
        "@searchSuggestions",
        (result) => {
          callback(result.value);
        }
  ...
  elements: {
    ...
    todoBoxTitleContent: {
      selector:
        "//div[contains(@class, 'todo-box') and div//h3/text() = '%s' and div//p/text()='%s']",
      locateStrategy: "xpath",
    },
    errorMessageCss: {
