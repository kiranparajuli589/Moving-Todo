...
$(document).on("click", searchSubmitButtonSelector, function (e) {
  ...
  let id = $(`${todoContentTitleSelector}:contains(${text})`)
    .parent()
    .parent()
    .parent()
    .parent()
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.message === "non-unique") {
        console.log(data);
        $(
          `<div id="${errorMessageSelector.slice(
            1
          )}">Todo with this Element title already exists.</div>`
      ...
      ele
        .find(todoBoxPKSelector)
        .html(`<i class="fas fa-fingerprint"></i> ${data.todo.id}`);
...
$(document).on("click", ".edit", async function (e) {
  ...
  const pk = await $(this)
    .parent()
    .parent()
    .find(todoBoxPKSelector)[0]
    .innerText.trim();
  const subject = await $(this)
    .parent()
    .parent()
    .find(todoContentTitleSelector)[0].innerHTML;
  const content = await $(this)
    .parent()
    .parent()
    .find(todoContentTextSelector)[0].innerHTML;
...
function validateTodoForm(subject, content, type = "create") {
  ...
  if (!subject) {
    ...
    if ($(subSel).next()[0].id !== errorMessageSelector.slice(1)) {
      ...
      //otherwise leave the empty subject msg and do nothing
      return 1;
    }
  }
  ...
  if (!content) {
    ...
    if ($(conSel).next().length === 0) {
      ...
      $(
        `<div id="${errorMessageSelector.slice(
          1
        )}">Please add content for your todo!</div>`
      ).insertAfter($(conSel));
      $(conSel).addClass(errOnInputClassSelector);
      return 1;
    } else {
      //otherwise leave the empty content msg and do nothing
      return 1;
    }
  }
